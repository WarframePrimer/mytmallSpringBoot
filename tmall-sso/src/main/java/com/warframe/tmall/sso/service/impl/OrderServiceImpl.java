...
public class OrderServiceImpl implements OrderService {
    ...
    public Order getOrder(Long orderId) {
        ...
        if (tbOrder == null) {
            throw new TmallException("通过id获取订单失败");
        }
    ...
    public int cancelOrder(Long orderId) {
        ...
        if (tbOrderMapper.updateByPrimaryKey(tbOrder) != 1) {
            throw new TmallException("取消订单失败");
        }
        return 1;
    }
    ...
    public Long createOrder(OrderInfo orderInfo) {
        ...
        if (member == null) {
            throw new TmallException("获取下单用户失败");
        }
        ...
        if (tbOrderMapper.insert(order) != 1) {
            throw new TmallException("生成订单失败");
        }
        ...
        for (CartProduct cartProduct : list) {
            ...
            orderItem.setNum(MathUtil.toIntExact(cartProduct.getProductNum()));
            ...
            orderItem.setTotalFee(MathUtil.toIntExact(cartProduct.getProductNum()) * cartProduct.getSalePrice());

            if (tbOrderItemMapper.insert(orderItem) != 1) {
                throw new TmallException("生成订单商品失败");
            }
        ...
        if (tbOrderShippingMapper.insert(orderShipping) != 1) {
            throw new TmallException("生成物流信息失败");
        }
    ...
    public int delOrder(Long orderId) {
        ...
        if (tbOrderMapper.deleteByPrimaryKey(String.valueOf(orderId)) != 1) {
            throw new TmallException("删除订单失败");
        }
    ...
    public String judgeOrder(TbOrder tbOrder) {
        ...
        if (tbOrder.getStatus() == 0) {
            ...
            if (days >= 1) {
                ...
                tbOrder.setCloseTime(new Date());
                if (tbOrderMapper.updateByPrimaryKey(tbOrder) != 1) {
                    throw new TmallException("更新订单失效失败");
                }
            } else {
                //返回到期时间
