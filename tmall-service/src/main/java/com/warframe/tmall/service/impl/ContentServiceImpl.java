...
import com.warframe.tmall.domain.dto.ContentDto;
import com.warframe.tmall.domain.dto.DtoUtil;
import com.warframe.tmall.domain.dto.front.HomeFloors;
...
import com.warframe.tmall.repository.*;
...
public class ContentServiceImpl implements ContentService {
    ...
    public int addContent(TbContent tbContent) {
        ...
        if(tbContentMapper.insert(tbContent)!=1){
            throw new TmallException("添加内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int deleteContent(Long id) {
        ...
        if(tbContentMapper.deleteByPrimaryKey(id)!=1){
            throw new TmallException("删除内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public int updateContent(TbContent tbContent) {
        ...
        if(tbContentMapper.updateByPrimaryKey(tbContent)!=1){
            throw new TmallException("更新内容失败");
        }
        //同步缓存
        deleteHomeRedis();
        return 1;
    ...
    public TbContent getContentById(Long id) {
        ...
        if(tbContent==null){
            throw new TmallException("通过id获取内容失败");
        }
        return tbContent;
    }
